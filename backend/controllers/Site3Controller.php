   ...
   /* public function actionIndex()
    {
      ...
      if ($request) {
        ...
        if ($value->save()) {
          if ($value->UploadMonth == $month && $value->SmsStatus == 0 && $value->errorMsg == '') {
            $value->SmsStatus=2;
...
return $this->redirect(['msme','id'=>$mon,'type'=>$type,'mon'=>$mon]);
 ...
 return $this->redirect(['mfi','id'=>$month,'type'=>$type,'mon'=>$mon]);
...
public function actionIndex()
{
 ...
 $result = $command->queryAll();
 $otherError=MsmeExcelData::find()->where(['RecordId'=>$model->RecordId,'IsDelete' => 0])->andWhere("BranchName='' or ClientId='' or Cluster='' or ClientName='' or DemandDate='' or NextInstallmentDate=''")
              ->count();
 ...
 if ($result) {
  $misscount=0;
  foreach($result as $miss_res){
   $misscount+=$miss_res['cnt'];
...
$month = date("m",strtotime($month));
  ...
  if($error){
   $details = MsmeExcelData::find()->where(['RecordId'=>$id,'IsDelete' => 0])
             ->andWhere("LoanAccountNo in (SELECT LoanAccountNo  FROM MsmeExcelData WHERE RecordId='$id'AND IsDelete = 0 GROUP BY LoanAccountNo HAVING COUNT(LoanAccountNo) > 1)")
             ->all();
  }
  else{
